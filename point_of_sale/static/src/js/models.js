...
exports.PosModel = Backbone.Model.extend({
    ...
    models: [
    {
        ...
        model:  'res.company',
        fields: [ 'currency_id', 'email', 'website', 'company_registry', 'vat', 'name', 'phone', 'partner_id' , 'country_id', 'tax_calculation_rounding_method', 'wifi', 'wifi_pass'],
        ...
        model:  'membership.package',
        fields: ['product_id', 'discount_type', 'discount', 'discount_to', 'name'],
        loaded: function(self,membership_package) {
            self.membership_package = membership_package;
        },
    },{
        model:  'membership.member',
        fields: ['package_id', 'partner_id', 'membership_start', 'membership_stop', 'discount'],
        loaded: function(self,membership_member){
            self.membership_member = [];
            self.membership_members = [];
            var nowDate = new Date().getTime();
            for (var i = 0; i < membership_member.length; i++) {
                if (new Date(membership_member[i].membership_start).getTime() <= nowDate && new Date(membership_member[i].membership_stop).getTime() >= nowDate) {
                    self.membership_member[membership_member[i].partner_id] = membership_member[i].package_id;
                    self.membership_members[membership_member[i].partner_id] = membership_member[i];
                }
            }
        },
    },{
        model:  'res.partner',
        fields: ['name','street','city','state_id','country_id','vat','phone','zip','mobile','email','barcode','write_date','property_account_position_id','membership_state','membership_start','membership_stop','membership_type','membership_discount','membership_state_type'],
        domain: [['customer','=',true]], 
        loaded: function(self,partners){
            var membership_member = self.membership_member;
            var membership_members = self.membership_members;
            var membership_package = self.membership_package;
            for (var i = 0; i < partners.length; i++) {
                if (membership_member[partners[i].id]) {
                    for (var j = 0; j < membership_package.length; j++) {
                        if (membership_package[j].product_id == membership_member[partners[i].id]) {
                            partners[i].membership_state_type = membership_package[j].name;
                            break;
                        }
                    }
                    partners[i].membership_start = membership_members[partners[i].id].membership_start;
                    partners[i].membership_stop = membership_members[partners[i].id].membership_stop;
                    partners[i].membership_discount = membership_members[partners[i].id].discount;
                }
            }
        ...
        fields: ['name','membership','type'],
        loaded: function(self,memberships){
            self.membership_type = [];
            for (var i = 0; i < memberships.length; i++) {
                if (memberships[i].membership && memberships[i].type=='memberships') {
                    self.membership_type.push({'id' : memberships[i].id, 'name': memberships[i].name});
                }
            }
        },
    },{
        model:  'product.template',
        fields: ['package_product_ids', 'type'],
        loaded: function(self,membership_package_product){
            self.membership_package_product = [];
            for (var i = 0; i < membership_package_product.length; i++) {
                if (membership_package_product[i].type == 'membership' && membership_package_product[i].package_product_ids.length > 0) {
                    for (var j = 0; j < membership_package_product[i].package_product_ids.length; j++) {
                        self.membership_package_product.push({
                            'package_id': membership_package_product[i].id,
                            'product_id': membership_package_product[i].package_product_ids[j]
                        });
                    }
                }
            }
        ...
        model:  'pos.permission',
        fields: ['user_id', 'perm_discount', 'perm_price'],
        loaded: function(self,pos_permission){
            self.pos_permission = [];
            for (var i = 0; i < pos_permission.length; i++) {
                self.pos_permission.push({
                    'user_id': pos_permission[i].user_id,
                    'discount': pos_permission[i].perm_discount,
                    'price': pos_permission[i].perm_price,
                });
            }
        },
    },{
    ...
    get_disable_button: function() {
        ...
        var chrome = new chrome.UsernameWidget();
        var user = chrome.get_name();
        return user;
        if(user){
            return user.name;
        }else{
...
exports.Orderline = Backbone.Model.extend({
    ...
    reload_discount: function () {
        var order = this.order;
    	if (order) {
            var package_product = this.pos.membership_package_product;
            ...
            if (lines) {
                ...
                if (client) {
                    ...
                    var membership_member = this.pos.membership_member;
                    if (membership_member[partner_id]) {
                        var package_id = membership_member[partner_id];
                        var packageMember = null;
                        for (var i = 0; i < this.pos.membership_package.length; i++) {
                            if (this.pos.membership_package[i].product_id == package_id) {
                                packageMember = this.pos.membership_package[i];
                                break;
                            }
                        }
                        if (packageMember) {
                            if (packageMember.discount_to == 'all') {
                                for (var i = 0; i < lines.length; i++) {
                                    if (lines[i].discount > 0) {
                                        continue;
                                    }
                                    // console.log(lines[i]);
                                    lines[i].discount = packageMember.discount;
                                    lines[i].discountStr = packageMember.discount;
                                }
                            }

                            if (packageMember.discount_to == 'specific') {
                                for (var i = 0; i < lines.length; i++) {
                                    if (lines[i].discount > 0) {
                                        continue;
                                    }
                                    for (var j = 0; j < package_product.length; j++) {
                                        if (package_product[j].package_id == package_id && package_product[j].product_id == lines[i].product.id) {
                                            lines[i].discount = packageMember.discount;
                                            lines[i].discountStr = packageMember.discount;
                                        }
                                    }
            ...
            //         }
            //         // console.log(lines[i]);
            //         lines[i].discount = client.membership_discount;
            //         lines[i].discountStr = client.membership_discount;
            //     }
            // }
        }
    },
    compute_all: function(taxes, price_unit, quantity, currency_rounding) {
        this.reload_discount();
        var self = this;
        var list_taxes = [];
                    ...
                    if (packageMember) {
                        if (packageMember.discount_to == 'all') {
                            for (var i = 0; i < lines.length; i++) {
                                ...
                                //     continue;
                                // }
                                lines[i].set_discount(packageMember.discount);
                            }
                        }
...
exports.NumpadState = Backbone.Model.extend({
    ...
    changeMode: function(newMode) {
        var cashier_id = window.posmodel.get_cashier().id;
        var pos_permission = window.posmodel.pos_permission;
        if (newMode == 'discount' || newMode == 'price') {
            if (newMode == 'discount') {
                var can_discount = false;
                for (var i = 0; i < pos_permission.length; i++) {
                    if (pos_permission[i].user_id[0] == cashier_id
                        && pos_permission[i].discount == true) {
                        can_discount = true;
                        break;
                    }
                }
