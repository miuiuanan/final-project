...
class MembershipLine(models.Model):
    ...
    # date_duration = fields.Integer(compute="_compute_duration", string="Duration")
    ...
    @api.depends('date', 'product.template.membership_duration')
    def _compute_duration(self):
        for line in self:
            self._cr.execute('''
            SELECT i.membership_duration FROM
            product_template i
            WHERE
            i.id = (
            SELECT l.invoice_id FROM
            ...
            ''', (line.membership_duration,))

            fetched = self._cr.fetchone()
            self.date_duration = fetched[0]
